...
public final class MineCache {
    ...
    public void start(CompletableFuture<HttpResponse<java.io.InputStream>> streamFuture) {
        if (running) return;
        running = true;
        thread = new Thread(() -> readStream(streamFuture), "FTE-MineStream");
        thread.setDaemon(true);
        thread.start();
        FteLogger.info("MineCache started");
    }
    ...
    public Map<String, JsonObject> getCachedMines() {
        ...
        while (running) {
            try {
                ...
                if (response.statusCode() != 200) {
                    FteLogger.warn("Mine stream HTTP " + response.statusCode() + ", retrying in 5s");
                    Thread.sleep(5000);
                    continue;
                }
                ...
                FteLogger.warn("Mine stream error, retrying in 5s: " + e.getMessage());
            }
            try { Thread.sleep(5000); } catch (InterruptedException ignored) {}
        }
        FteLogger.info("MineCache stopped");
    }
    ...
    private void processSseData(String json) {
        try {
            ...
            cache.clear();
            for (var item : items) {
                JsonObject mine = item.getAsJsonObject();
                String key = mine.get("server_id").getAsInt() + "_" + mine.get("rarity").getAsString();
                cache.put(key, mine);
            }
        } catch (Exception e) {
            FteLogger.debug("SSE mine parse error: " + e.getMessage());
        }
    }
}
